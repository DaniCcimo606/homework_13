...
const webTitle = document.querySelector(".logo-title");
...
let count = document
    ...
    .querySelectorAll(".nav__item").length;
console.log(b);

for (let i = 0; i < count; i++) {
    let c = document.querySelectorAll(".nav__link")[i];
    let d = c.href;
    console.log(d);
    if (d == b) {
        c.classList.add("nav__link_target");
    } else if (document.title == "Oksana Kobzar") {
        document
            .querySelectorAll(".nav__link")[2]
            .classList.add("nav__link_target");
    } else {
        c.classList.remove("nav__link_target");
    }
}

// Отмена перехода к табу при нажатии
if (document.title == "Home") {
    let tabs = document.querySelectorAll(".tab__link, .slide-btn");
    let tabsContent = document.querySelectorAll(
        ".tab__item-content, .slider__item"
    );
    tabs.forEach((tab) => {
        tab.addEventListener("click", function (e) {
            ...
            tabsContent.forEach((tabItem) => {
                tabItem.classList.remove(
                    "tab__item-content--target",
                    "slider__item--target"
                );
            });
            tab.classList.add("tab__link--target", "slide-btn--target");
            document
                .getElementById(id)
                .classList.add(
                    "tab__item-content--target",
    ...
    document.querySelector(".slide-btn").click();
}
